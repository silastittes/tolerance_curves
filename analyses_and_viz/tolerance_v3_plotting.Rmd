...
tab_strings <- c("$\\zeta$", "$\\delta$", "$\\epsilon$", "maxima", "$\\epsilon - \\delta$", "area", "$\\zeta/(\\epsilon - \\delta)$")
...
cairo_pdf("figures/fig2_signal.pdf")
...
for(spp in spp_list){
  ...
  lines(mean_line[[1]], mean_line[[2]], 
        lwd = 2, col = colz[state_reg_aqua_terr2terr[spp]])
...
pdf("figures/A9_fitted_v_resid.pdf")
...
param_letters <- c("α_t", "β_t", "ζ_t", "δ_t", "ε_t", "β_0", "β_1", "ν") %>% as_tibble
...
cairo_pdf(filename = paste0("figures/A", 1, "_", spp_par[1], "_prior_post.pdf"))
plt_params(p = 1, den_rng = c(-6,6), -1, 1)
dev.off()

cairo_pdf(filename = paste0("figures/A", 2, "_", spp_par[2], "_prior_post.pdf"))
plt_params(p = 2, den_rng = c(-6,6), -1, 1)
dev.off()

cairo_pdf(filename = paste0("figures/A", 3, "_", spp_par[3], "_prior_post.pdf"))
plt_params(p = 3, den_rng = c(-3,3), 0, 1) 
dev.off()

cairo_pdf(filename = paste0("figures/A", 4, "_", spp_par[4], "_prior_post.pdf"))
plt_params(p = 4, den_rng = c(-3,3), 0, 1) 
dev.off()

cairo_pdf(filename = paste0("figures/A", 5, "_", spp_par[5], "_prior_post.pdf"))
plt_params(p = 5, den_rng = c(-3,3), 0, 1) 
dev.off()

cairo_pdf(filename = paste0("figures/A", 6, "_", spp_par[6], "_prior_post.pdf"))
plt_params(p = 6, den_rng = c(-3,3), 0, 1) 
dev.off()

cairo_pdf(filename = paste0("figures/A", 7, "_", spp_par[7], "_prior_post.pdf"))
plt_params(p = 7, den_rng = c(-3,3), 0, 1) 
dev.off()

cairo_pdf(filename = paste0("figures/A", 8, "_", spp_par[8], "_prior_post.pdf"))
plt_params(p = 8, den_rng = c(-3,3), 2, 1) #+ theme(legend.position="none")
dev.off()


#making sense of transforms
#matplot(1 - 0.2*posts$b_t, log(posts$b - 1))
#abline(0,1)
