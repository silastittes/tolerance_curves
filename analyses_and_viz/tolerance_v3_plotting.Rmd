...
legend("topright", c("unscaled","scaled"), pch = 21, 
...
get_ouwie_mat <- function(file_name){
  ...
  cOuwie_mat <- cbind(
    matrix(unlist(uno), ncol = 1),
    matrix(unlist(dose), ncol = 1),
    matrix(unlist(tres), ncol = 1)
  )
...
int_oum_sim <- get_ouwie_mat(file_name = "derived_files/c_OUM_simmap.txt")


den_ou <- apply(int_oum_sim, 2, density)
xrng <- range(sapply(den_ou, function(x) range(x$x)))
yrng <- range(sapply(den_ou, function(x) range(x$y)))

hdis <- apply(int_oum_sim, 2, HDI, percent = 0.95)
xrng <- range(hdis)*c(0.8, 1.2)
yrng <- range(sapply(den_ou, function(x) range(x$y)))


colz <- as.color(x = 1:3, alpha = 0.5)
