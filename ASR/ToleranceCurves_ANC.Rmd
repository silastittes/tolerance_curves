...
draw_list <- as.list(1:(ndraws))
sppint_list <- as.list(1:max(emery$sppint))
curveK_draws <- sapply(draw_list, function(pr){
...
plot(density(curveK_draws))
lines(density(curveK_draws_scaled))
...
### SPECIES ORDER IN PARAMETER MATRICES DOES NOT MATCH PHYLO ORDER
...
ouwie_maxima_ou1 <- ouwie_draws(df = maximadf, draws = 100, mod = "OU1")
ouwie_maxima_oum <- ouwie_draws(df = maximadf, draws = 100, mod = "OUM")

mean(sapply(ouwie_maxima_ou1, function(x) x$AIC) > 
       sapply(ouwie_maxima_oum, function(x) x$AIC))

plot_ouwie_draws(ouwie_maxima_oum)
compare_ouwie_draws(ouwie_maxima_oum)

ouwie_c_ou1 <- ouwie_draws(df = posts$c, draws = 100, mod = "OU1")
ouwie_c_oum <- ouwie_draws(df = posts$c, draws = 100, mod = "OUM")

mean(sapply(ouwie_c_ou1, function(x) x$AIC) >
       sapply(ouwie_c_oum, function(x) x$AIC))

plot_ouwie_draws(ouwie_c_oum)
compare_ouwie_draws(ouwie_c_oum)

ouwie_e1_ou1 <- ouwie_draws(df = posts$e1, draws = 100, mod = "OU1")
ouwie_e1_oum <- ouwie_draws(df = posts$e1, draws = 100, mod = "OUM")

mean(sapply(ouwie_e1_ou1, function(x) x$AIC) >
       sapply(ouwie_e1_oum, function(x) x$AIC))

plot_ouwie_draws(ouwie_e1_oum)
compare_ouwie_draws(ouwie_e1_oum)

WWWusagexxouwie_d_ou1 <- ouwie_draws(df = posts$d, draws = 100, mod = "OU1")
ouwie_d_oum <- ouwie_draws(df = posts$d, draws = 100, mod = "OUM")

mean(sapply(ouwie_d_ou1, function(x) x$AIC) < 
       sapply(ouwie_d_oum, function(x) x$AIC))

plot_ouwie_draws(ouwie_d_oum)
compare_ouwie_draws(ouwie_d_oum)
