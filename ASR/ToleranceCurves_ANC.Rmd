...
#assign node habitat state to highest prob
simstates <- lapply(reg_fit, function(x) states[apply(x$mapped.edge, 1, which.max)])
...
ouwie_maxima_ou1 <- ouwie_draws(df = maximadf, draws = 2, mod = "OU1", tree = lasth, sims =  hab_sims)

ouwie_maxima_oum <- ouwie_draws(df = maximadf, draws = 2, mod = "OUM", tree = lasth, sims = hab_sims)


#!!! OU1 model is identical across trees...odd??
unlist(lapply(get_thetas(ouwie_maxima_ou1), function(x)x[1,]))


#get all aquatic-terr into a single vector 
unlist(lapply(get_thetas(ouwie_maxima_oum), function(x)x[1,]))
#get all terrestrial into a single vector 
unlist(lapply(get_thetas(ouwie_maxima_oum), function(x)x[2,]))
#get all vernal into a single vector
unlist(lapply(get_thetas(ouwie_maxima_oum), function(x)x[3,]))
