...
parameters {
  ...
  #hyper priors
  real <lower=2> hyper_a;
  real <lower=0> sigma_a;
  
  real <lower=2> hyper_b;
  real <lower=0> sigma_b;
  
  real <lower=0> hyper_c;
...
model {
  ...
  hyper_a ~ normal(2,4);
  sigma_a ~ normal(0,4);
  a ~ normal(hyper_a, sigma_a);
    
  hyper_b ~ normal(2,4);
  sigma_b ~ normal(0,4);
  b ~ normal(hyper_b, sigma_b);
  
  hyper_c ~ normal(1,4);
  sigma_c ~ normal(0,4);
  c ~ normal(hyper_c, sigma_c);
  
  hyper_d ~ normal(minx,4);
  sigma_d ~ normal(0,4);
  d ~ normal(hyper_d, sigma_d);
  
  hyper_e ~ normal(maxx, 4);
  sigma_e ~ normal(0,4);
