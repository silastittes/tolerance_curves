...
biomass_max <- by(data = emery$Inflor_biomass, INDICES = emery$Species, FUN = max)
...
stan.fit <- stan(file = "tolerance_zig_v3_nohier.stan",
                 ...
                 iter = 10000, chains = 4,
                 pars = watch,
                sample_file = "tolerance_zig_v2.samples",
                diagnostic_file = "tolerance_zig_v2.diag"
)
...
denA <- density(posts$a[,1])
plot(denA$x, denA$y, xlim=range(posts$a), ylim=c(0,0.9),type="n")
...
denB <- density(posts$b[,1])
plot(denB$x, denB$y, xlim=range(posts$b), type="n", ylim=c(0,2))
...
denC <- density(posts$c)
plot(denC$x, denC$y, xlim=range(posts$c), type="n", ylim=c(0,1.2))
...
denE1 <- density(posts$hyper_e - posts$hyper_d)
plot(denC$x, denC$y, xlim=range(posts$e1), type="n", ylim=c(0,1))
polygon(denE_pr, col="red")
polygon(denE1, col="dodgerblue")
for(i in 1:dataList$numSpp){
