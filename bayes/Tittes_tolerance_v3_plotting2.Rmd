...
nn <- 10000
priorDens$a <- density(rtruncnorm(nn, mean = 2, sd = 2, a = 1))
priorDens$b <- density(rtruncnorm(nn, mean = 2, sd = 2, a = 1))
priorDens$c <- density(rtruncnorm(nn, mean = 2, sd = 2, a = 0))
...
for(i in namez){
  ...
  if(length(dim(cPost)) < 2){
    ...
    polygon(priorDens[[i]], col="grey60", border = F)
    mtext(text = i, side = 3)
  } else {
